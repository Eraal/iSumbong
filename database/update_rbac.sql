-- RBAC Update for iReport System
-- Run this in phpMyAdmin to add RBAC functionality

-- Step 1: Add role column
ALTER TABLE `users` ADD COLUMN `role` VARCHAR(50) NOT NULL DEFAULT 'user' AFTER `status`;

-- Step 2: Add verification columns
...
-- Step 2: Update existing users to have 'user' role and verified status
UPDATE `users` SET `role` = 'user', `is_verified` = 1 WHERE `role` IS NULL OR `role` = '';

-- Step 3: Migrate admin data to users table with admin role
...
-- Step 5: Create role changes audit table
...
-- Step 6: Create user permissions table (optional for future expansion)
